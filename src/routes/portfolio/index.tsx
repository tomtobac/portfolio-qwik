...
export default component$(() => {
  ...
  const pruebaClick = $(async () => {
    ...
    const repos = await fetch(`https://api.github.com/users/ArturoZarzalejo/repos`)
    ...
    state.github.repos = repos.sort((a, b) => b.pushed_at.localeCompare(a.pushed_at));
    ...
    <div class="container container-center">
      ...
      <div class="icons-socials">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="445" height="333" viewBox="0 0 445 333" fill="none">
          ...
          <path d="M193.79 213.352L267.158 166.503L193.79 119.65V213.352Z" fill="white" />
        </svg>
      </div>

      <ul class="repos-list-container">
        {state.github.repos && state.github.repos.map((repo) =>
          <li key={repo.id} onClick$={() => fetch(`${repo.url}/readme`)
            .then(data => data.json())
            .then(data => { console.log(data, atob(data.content)) })}>{repo.name}</li>
        )}
